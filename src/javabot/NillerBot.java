...
public class NillerBot implements BWAPIEventListener {
	
	// Some miscelaneous variables. Feel free to add yours.
	int homePositionX;
	int homePositionY;
	List<UnitTypes> buildOrder = new ArrayList<UnitTypes>();
	int buildings = 1;

	private JNIBWAPI bwapi;
	private boolean isWaitingForBuilding = false;
	private boolean autoPylons = false;
	...
	public void gameStarted() {		
		...
		chooseBuildOrder(Race.fromID(bwapi.getSelf().getRaceID()));
	...
	private void chooseBuildOrder(Race race) {
		
		if (race == Race.PROTOSS){
			buildOrder.add(UnitTypes.Protoss_Probe);
			buildOrder.add(UnitTypes.Protoss_Probe);
			buildOrder.add(UnitTypes.Protoss_Pylon);
			buildOrder.add(UnitTypes.Protoss_Probe);
			buildOrder.add(UnitTypes.Protoss_Gateway);
			buildOrder.add(UnitTypes.Protoss_Probe);
			buildOrder.add(UnitTypes.Protoss_Gateway);
			buildOrder.add(UnitTypes.Protoss_Zealot);
			buildOrder.add(UnitTypes.Protoss_Dark_Archon);
	...
	public void act() {
		...
		if (buildOrder.get(0) == UnitTypes.Protoss_Dark_Archon) {
			autoPylons = true;
			removeFromBuildOrder();
		}
		...
		for (Unit unit : bwapi.getMyUnits()) {
			...
			if (unit.getTypeID() == UnitTypes.Protoss_Nexus.ordinal()) {
				...
				if (buildOrder.get(0) == UnitTypes.Protoss_Probe) {
					// check if we have enough minerals and supply, and (if we do) train one worker (Terran_SCV)
					if ((bwapi.getSelf().getMinerals() >= 50) && !isWaitingForBuilding){
			...
			// Train zealot
			if (unit.getTypeID() == UnitTypes.Protoss_Gateway.ordinal() && !unit.isBeingConstructed()) {
				if (buildOrder.get(0) == UnitTypes.Protoss_Zealot) {
					if (bwapi.getSelf().getMinerals() >= 100 && unit.getTrainingQueueSize() == 0) {
						bwapi.train(unit.getID(), UnitTypes.Protoss_Zealot.ordinal());
						System.out.println("train zealot");
						removeFromBuildOrder();
					}
				}
			}
		...
		// And let's build some Pylons if we are low on supply (if free supply is less than 4).
		if (buildOrder.get(0) == UnitTypes.Protoss_Pylon || (((bwapi.getSelf().getSupplyTotal() - bwapi.getSelf().getSupplyUsed())/2) < 4) && autoPylons) {
			// Check if we have enough minerals,
			if (bwapi.getSelf().getMinerals() >= 100  && !isWaitingForBuilding) {
		...
		} else if (buildOrder.get(0) == UnitTypes.Protoss_Gateway) {
			// Check if we have enough minerals,
			if (bwapi.getSelf().getMinerals() >= 150 && !isWaitingForBuilding) {
				// try to find the worker near our home position
				int worker = getNearestUnit(UnitTypes.Protoss_Probe.ordinal(), homePositionX, homePositionY);
				if (worker != -1) {
					// if we found him, try to select appropriate build tile position for supply depot (near our home base)
					Point buildTile = getBuildTile(worker, UnitTypes.Protoss_Gateway.ordinal(), homePositionX, homePositionY);
					// if we found a good build position, and we aren't already constructing a Supply Depot, 
					// order our worker to build it
					if (buildTile.x != -1) {
						bwapi.build(worker, buildTile.x, buildTile.y, UnitTypes.Protoss_Gateway.ordinal());
						System.out.println("build gateway");
						removeFromBuildOrder();
						isWaitingForBuilding = true;
					}
				}
	...
	private void attackEnemyBase(Unit unit) {

		for (BaseLocation b : bwapi.getMap().getBaseLocations()) {
			// If this is a possible start location,
			if (b.isStartLocation() && distance(b.getX(), b.getY(), homePositionX, homePositionY) > 500) {
				
				bwapi.attack(unit.getID(), b.getX(), b.getY());
	...
	private void recountBuildings() {
		int i = 0;
		for (Unit unit : bwapi.getMyUnits()) {
			if (unit.getTypeID() == UnitTypes.Protoss_Pylon.ordinal() || 
					unit.getTypeID() == UnitTypes.Protoss_Gateway.ordinal() || 
	...
	private void removeFromBuildOrder() {
		List<UnitTypes> newBuildOrder = new ArrayList<UnitTypes>();
		int i=0;
		for(UnitTypes ut : buildOrder){
			if (i!=0)
				newBuildOrder.add(ut);
			i++;
		}
		
		buildOrder = newBuildOrder;

		System.out.println("next unit:" + buildOrder.get(0));
	}
